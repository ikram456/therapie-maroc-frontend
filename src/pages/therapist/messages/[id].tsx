...
export default function TherapistChatPage() {
  ...
  const patientName = connection?.patient ? `${connection.patient.firstName} ${connection.patient.lastName}` : 'Patient';
        ...
        <div className="flex-1 overflow-y-auto p-4 max-w-4xl mx-auto w-full">
          <div className="space-y-3">
            {messages?.map((msg: any) => (
              <div
                key={msg.id}
                className={`flex ${msg.senderRole === 'THERAPIST' ? 'justify-end' : 'justify-start'}`}
              >
                <div
                ...
                >
                  <p>{msg.content}</p>
                  <p className={`text-xs mt-1 ${msg.senderRole === 'THERAPIST' ? 'text-white/70' : 'text-gray-400'}`}>
